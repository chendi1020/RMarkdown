...
####Distribution of first term GPA by 6yr graduation and race
```{r,results='asis', echo=FALSE,fig.align = 'center', dpi = 100, tidy = F}
library(plotly)
library(dplyr)

ds <- PAG %>% filter(COHORT==2010 & ! is.na(FIRSTTERMGPA) ) %>%
        select(PID, FIRSTTERMGPA, GRAD6, ETHNICITY)

p <- plot_ly(ds, x = ~GRAD6, y = ~FIRSTTERMGPA, color = ~ETHNICITY, type = "box") %>%
